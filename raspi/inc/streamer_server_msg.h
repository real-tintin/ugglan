...
#ifndef STREAMER_SERVER_MSG_H
#define STREAMER_SERVER_MSG_H
...
public:
    ...
    json as_json()
    {
        ...
        {
            j = json({
                {_get_metadata_key(), static_cast<int>(_metadata)},
                {_get_data_key(), _data}
            });
        }
...
protected:
    const T _get_metadata() { return _metadata; }

    virtual const std::string _get_metadata_key() { std::logic_error("Not implemented"); return ""; }
private:
    T _metadata;
    json _data;
    bool _valid;

    const std::string _get_data_key() { return "data"; }
};
...
public:
    ...
    const Method method() { return _get_metadata(); }
protected:
    const std::string _get_metadata_key() override { return "method"; }
};
...
public:
    ...
    const Code code() { return _get_metadata(); }
protected:
    const std::string _get_metadata_key() override { return "code"; }
};
} /* msg */
} /* streamer */

#endif /* STREAMER_SERVER_MSG_H */
