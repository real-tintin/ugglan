#ifndef DATA_LOG_QUEUE_HPP
#define DATA_LOG_QUEUE_HPP
...
class DataLogQueue {
  public:
    ...
    template <typename T> void last_signal_data(T *data, DataLogSignal signal, T default_data = 0);
  ...
  private:
    ...
    template <typename T> DataLogType _get_data_type(T data);

    template <typename T> void _update_last_signal_map(T data, DataLogSignal signal);
...
template <typename T> void DataLogQueue::last_signal_data(T *data, DataLogSignal signal, T default_data)
{
    const std::lock_guard<std::mutex> lock(_mutex);
...
template <typename T> DataLogType DataLogQueue::_get_data_type(T data)
{
    DataLogType type;
...
template <typename T> void DataLogQueue::_update_last_signal_map(T data, DataLogSignal signal)
{
    std::memcpy(&_last_signal_map[signal], &data, sizeof(T));
}

#endif /* DATA_LOG_QUEUE_HPP */
