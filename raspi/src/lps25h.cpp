...
double Lps25h::get_pressure()
{
    ...
    int32_t raw_pres = static_cast<int32_t>((buf[LPS25H_BUF_PRESS_OUT_H] << 16) | (buf[LPS25H_BUF_PRESS_OUT_L] << 8) |
                                            buf[LPS25H_BUF_PRESS_OUT_XL]);
...
double Lps25h::get_temperature()
{
    ...
    int16_t raw_temp = static_cast<int16_t>((buf[LPS25H_BUF_TEMP_OUT_P_H] << 8) | buf[LPS25H_BUF_TEMP_OUT_P_L]);
