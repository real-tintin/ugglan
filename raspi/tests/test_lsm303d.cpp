...
static I2cWriteMap WRITE_MAP = {
    LSM303D_REG_CTRL1, LSM303D_REG_CTRL2, LSM303D_REG_CTRL5, LSM303D_REG_CTRL6, LSM303D_REG_CTRL7};

void set_lsm303d_stub_data(I2cConnStub &i2c_conn, uint8_t acc[LSM303D_BUF_ACC_SIZE], uint8_t mag[LSM303D_BUF_MAG_SIZE])
{
    I2cReadBlockMap read_map = {{LSM303D_REG_OUT_X_L_A | POLOLU_AUTO_INCREMENT, acc},
...
TEST_CASE("lsm303d interpretation")
{
    ...
    SECTION("non-zero")
    {
        uint8_t acc_data[LSM303D_BUF_ACC_SIZE] = {123,
                                                  0, // 123 * 4 * 9.82 / (2^15-1) = 0.14745
                                                  133,
                                                  255, // -123 * 4 * 9.82 / (2^15-1) = -0.14745
                                                  0,
                                                  32}; // 8192 * 4 * 9.82 / (2^15-1) = 9.8203
        uint8_t mag_data[LSM303D_BUF_MAG_SIZE] = {225,
                                                  16, // 4321 * 4 / (2^15-1) = 0.52748
                                                  31,
                                                  239, // -4321 * 4 / (2^15-1) = -0.52748
                                                  0,
                                                  80}; // 20480 * 4 / (2^15-1) = 2.5001
